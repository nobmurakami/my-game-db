...
- [工夫したポイント](#工夫したポイント)
- [使用技術（開発環境）](#使用技術（開発環境）)
- [課題・今後実装したい機能](#課題・今後実装したい機能)
- [ローカルでの動作方法(Dockerを使用しない場合)](#ローカルでの動作方法(Dockerを使用しない場合))
- [DB設計](#DB設計)
...
ログインすることでゲーム情報の新規登録・編集ができるとともに、マイリスト機能でゲームを管理することができます。<br>
このアプリケーションはRuby on Railsで制作されており、次のような機能が含まれています。
- ユーザー認証機能(devise)
- 一つのフォームから複数のテーブルへの保存・更新(Formオブジェクトパターン)
- トランザクション
- 多対多の関係
- 列挙型(Enum)
- 記事の詳細検索機能と並び替え機能(ransack)
- Ajaxでの「お気に入り」機能
- Ajaxでのタグ付け機能
- ページネーション（kaminari）
- パンくずリスト(gretel)
- Action Mailerでのメール送信
- 単体テスト/結合テスト(RSpec)
- Dockerでの開発環境構築
- S3への画像アップロード
- EC2への自動デプロイ(Capistrano)
...
# 工夫したポイント
- 「開発元」と「パブリッシャー」はどちらも会社名を扱うカテゴリーなので、テーブルを分けずに列挙型(Enum)を利用したアソシエーションで区別するようにし、会社名で検索することで「開発元」「パブリッシャー」のどちらにもヒットするようにした。
...
## インフラ
AWS(EC2, S3, Route 53), Capistrano, Docker(開発環境)
...
- 複数登録できるカテゴリにおける逐次検索機能： すでにDBに存在するジャンルなどを入力しやすいように。
